    ...
    case "GOT_CURRENT_PROJECT":
      {
        ...
        var currentProject = _extends({}, state.currentProject);
        if (isShared === "?true") {
          var data = [].concat(_toConsumableArray(state.sharedFiles));
          for (var i = 0; i < data.length; i++) {
            if (data[i].id === id) {
              currentProject.name = data[i].name;
              currentProject.data = data[i].data;
              currentProject.map = data[i].map;
              currentProject.isProject = data[i].isProject;
              return _extends({}, state, {
                currentFile: currentFile,
                isSettingCurrent: false,
                associativeFiles: [],
                currentProjectSchema: currentProject.map
              });
            }
          }
        } else {
          ...
          for (var _i = 0; _i < _data.length; _i++) {
            if (_data[_i].id === id) {
              currentProject.name = _data[_i].name;
              currentProject.data = _data[_i].data;
              currentProject.map = _data[_i].map;
              currentProject.isProject = _data[_i].isProject;
...
//# sourceMappingURL=0.6e754a63edefe208fdda.hot-update.js.map